To use this project follow the below steps:
1)	Create new directory in your local file system then start CMD from there
2)	Clone the repository to your local
git clone https://github.com/alhariri-hashem/test_automation_project.git
3)	Execute this command
py explore.py --testId <enter the test id from testrail>

!(Demo)(https://github.com/alhariri-hashem/test_automation_project/blob/Demo1/Demo1.gif)
